...
public class AuthController {
    ...
    public ResponseEntity<AuthResponse> unlock(@RequestBody UnlockRequest request) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new AuthResponse( null, null,"No se pudo enviar correo"));
        }
    }

    @PostMapping(value = "register")
    public ResponseEntity<AuthResponse> register(@RequestBody @Validated RegisterRequest request, BindingResult bindingResult) throws MessagingException, CustomException {
        ...
        if (token != null) role = jwtService.getClaim(token, claims -> claims.get("role", String.class));

        if (role ==null) role = Role.CLIENT.name();
        ...
        if  (!((role.equals(Role.CLIENT.name()) && request.getRole().equals(Role.CLIENT.name())) ||
            ...
            return new ResponseEntity<>(new AuthResponse(null, null, "Su rol no puede realizar esta acción"), HttpStatus.BAD_REQUEST);
        }
