...
CREATE TABLE IF NOT EXISTS `FOURPARKSDATABASE`.`USER_AUTHENTICATION` (
  ...
  PRIMARY KEY (`FK_IDUSER`, `FK_IDDOCTYPE`),
...
CREATE TABLE IF NOT EXISTS `FOURPARKSDATABASE`.`ADMINISTRATOR` (
  ...
  `FK_IDDOCTYPE` VARCHAR(3) NOT NULL,
  PRIMARY KEY (`FK_IDUSER`, `FK_IDDOCTYPE`),
  CONSTRAINT `FK_ADMINISTRADOR_USER`
    FOREIGN KEY (`FK_IDUSER` , `FK_IDDOCTYPE`)
    REFERENCES `FOURPARKSDATABASE`.`USER` (`IDUSER` , `FK_IDDOCTYPE`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `FOURPARKSDATABASE`.`CARD` (
  ...
  `FK_IDDOCTYPE` VARCHAR(3) NOT NULL,
  PRIMARY KEY (`IDCARD`),
  INDEX `FK_CARD_CLIENTE_IDX` (`FK_IDUSER` ASC, `FK_IDDOCTYPE` ASC) VISIBLE,
  CONSTRAINT `FK_CARD_CLIENTE`
    FOREIGN KEY (`FK_IDUSER` , `FK_IDDOCTYPE`)
    REFERENCES `FOURPARKSDATABASE`.`CLIENT` (`FK_IDUSER` , `FK_IDDOCTYPE`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = INNODB
DEFAULT CHARACTER SET = UTF8MB3;
...
CREATE TABLE IF NOT EXISTS `FOURPARKSDATABASE`.`PARKING` (
  ...
  CONSTRAINT `FK_PARKING_ADMINISTRADOR`
    ...
    REFERENCES `FOURPARKSDATABASE`.`ADMINISTRATOR` (`FK_IDUSER` , `FK_IDDOCTYPE`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION
) ENGINE = INNODB
DEFAULT CHARACTER SET = UTF8MB3;
...
CREATE TABLE IF NOT EXISTS `FOURPARKSDATABASE`.`RESERVATION` (
  ...
  INDEX `FK_RESERVATION_PARKING_IDX` (`FK_IDPARKING` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `FOURPARKSDATABASE`.`PARKINGSPACE` (
  ...
  INDEX `FK_PARKINGSPACE_PARKINGTYPE_IDX` (`FK_IDPARKINGTYPE` ASC) VISIBLE,
  CONSTRAINT `FK_PARKINGSPACE_PARKING_UNIQUE`
    UNIQUE (`IDPARKINGSPACE`,`FK_IDPARKING`,`FK_IDCITY`,`FK_IDVEHICLETYPE`, `FK_IDPARKINGTYPE`),
...
CREATE TABLE IF NOT EXISTS `FOURPARKSDATABASE`.`RATE` (
  ...
  `FK_IDPARKING` INT NOT NULL,
  `FK_IDCITY` INT NOT NULL,
  `FK_IDVEHICLETYPE` VARCHAR(3) NOT NULL NOT NULL,
  `FK_IDPARKINGTYPE` VARCHAR(3) NOT NULL,
  PRIMARY KEY (`FK_IDPARKING`,`FK_IDCITY`,`FK_IDVEHICLETYPE`,`FK_IDPARKINGTYPE`),
  ...
  INDEX `FK_RATE_PARKINGTYPE_IDX` (`FK_IDPARKINGTYPE` ASC) VISIBLE,
